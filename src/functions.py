...
#from sklearn import SVD, Dataset, accuracy, BaselineOnly, Reader, KNNWithMeans, KNNBasic, NormalPredictor
import re
...
def stem_tag_docs(docs, my_stop_words, id_col='id'):
    ls = LancasterStemmer()
    results = docs.apply(lambda r: TaggedDocument(words=preprocessor_and_stem(r['text'], my_stop_words), tags=[str(r[id_col])]), axis=1)
    return results.tolist()
